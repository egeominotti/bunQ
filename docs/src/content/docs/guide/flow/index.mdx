...
description: 'Build durable job graphs in bunqueue: children run first, then the parent reads their results. Flows are atomic in SQLite and PostgreSQL modes.'
head:
  - tag: meta
    attrs:
      property: og:image
...
<div class="bq-wrap bq-hero">
  ...
  <h1 class="bq-hero-h1 bq-bench-h1">
    Jobs that <em>wait for each other.</em>
  </h1>
  <p class="bq-hero-sub">
    Some work only makes sense in order: resize every image, then build the album; charge each line,
    then close the invoice. A flow declares that shape once and bunqueue holds the parent until its
    children are done.
  </p>
</div>
