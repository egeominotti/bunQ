 ...
 * and large fan-out scenarios. These tests verify execution order and completion;
...
async function main() {
  ...
  console.log('1. Testing DEEP CHAIN (10 steps)...');
  {
    ...
    if (executionOrder.length === STEP_COUNT) {
      ...
      if (inOrder) {
        ok(
          `Deep chain: all ${STEP_COUNT} steps executed in order (${executionOrder.join(' -> ')})`
        );
      } else {
      ...
      fail(
        `Expected ${STEP_COUNT} steps, got ${executionOrder.length}: [${executionOrder.join(', ')}]`
      );
    }
  ...
  console.log('\n2. Testing MULTI-QUEUE FLOW (3 queues)...');
  {
    ...
    if (executionOrder.length === 6) {
      ...
      for (let i = 0; i < 6; i++) {
        if (
          executionOrder[i].queue !== expected[i].queue ||
          executionOrder[i].step !== expected[i].step
        ) {
          match = false;
          break;
        }
      ...
      fail(
        `Expected 6 steps, got ${executionOrder.length}: [${executionOrder.map((e) => `${e.queue}:${e.step}`).join(', ')}]`
      );
    }
  ...
  console.log('\n3. Testing FAN-OUT FAN-IN (5 parallel + 1 final)...');
  {
    ...
    if (
      finalExecuted &&
      parallelTasks.length === PARALLEL_COUNT &&
      aggregateIdx === executionOrder.length - 1
    ) {
      ok(
        `Fan-out fan-in: ${parallelTasks.length} parallel tasks, aggregate last (idx ${aggregateIdx}/${executionOrder.length - 1})`
      );
    } else {
      fail(
        `finalExecuted=${finalExecuted}, parallel=${parallelTasks.length}, aggregateIdx=${aggregateIdx}, order=[${executionOrder.join(', ')}]`
      );
    }
  ...
  console.log('\n4. Testing MULTIPLE INDEPENDENT CHAINS (10 x 3 steps)...');
  {
    ...
    if (completedChains === CHAIN_COUNT) {
      ...
      if (allInOrder) {
        ok(
          `All ${CHAIN_COUNT} chains completed with ${STEPS_PER_CHAIN} steps each in correct order`
        );
      } else {
        const details = Array.from(chainResults.entries())
          .map(([id, s]) => `chain${id}=[${s.join(',')}]`)
          .join(', ');
  ...
  console.log('\n5. Testing CHAIN WITH DELAYED STEP...');
  {
    ...
    if (executionTimestamps.length === 3) {
      ...
      if (orderCorrect && delayRespected && step2AfterStep1) {
        ok(
          `Chain with delay: order correct, step-1 delayed by ${step1Time - startTime}ms (>=250ms), step-2 after step-1`
        );
      } else {
        fail(
          `orderCorrect=${orderCorrect}, delayRespected=${delayRespected} (${step1Time - startTime}ms), step2AfterStep1=${step2AfterStep1}`
        );
      }
    } else {
  ...
  console.log('\n6. Testing LARGE FAN-OUT (20 parallel + 1 final)...');
  {
    ...
    if (finalExecuted && completedParallel.size === PARALLEL_COUNT) {
      ...
      fail(
        `finalExecuted=${finalExecuted}, parallelCompleted=${completedParallel.size}/${PARALLEL_COUNT}`
      );
    }
