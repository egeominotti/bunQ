...
`buildClientTls` (`connection.ts:37-46`): `undefined`/`false` → `undefined` (plaintext, the default); `true` → `true` (TLS with system CAs); object form → `{ rejectUnauthorized?, ca: Bun.file(caFile)? }` (only the provided keys are set). The result is spread into `Bun.connect` (`connection.ts:144-148`). The connection-pool dedup key incorporates a serialized form of the TLS option (`shared.ts:20-21`), so callers with different TLS configs never share a socket.
