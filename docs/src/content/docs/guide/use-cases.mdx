...
*PHP, Rust and Elixir report progress the same way (`updateProgress` / `update_progress`), see the [Worker guide](/guide/worker/#use-the-job-object).*
...
payments.add(
    "charge",
    {"orderId": "ORD-123", "amount": 9999, "idempotencyKey": "order-ORD-123"},
    job_id="charge-ORD-123", durable=True,  # no duplicate, no loss on crash
    attempts=3, backoff=5000, timeout=60_000,
)
```
