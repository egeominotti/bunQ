...
| `bench/fix-impact.ts` + `fix-impact/*`                   | Before/after recovery, queries, scheduling, indexes, waiters, heap retention     | Publication-grade cross-revision harness; correctness travels with timing and a failed exact oracle marks that result non-comparable. `--only=pull-group-head-of-line` isolates the saturated-tenant FIFO-group case; `--only=queue-paths` separately guards ungrouped and balanced mixed admission/claim costs, including disclosed regressions. |
...
| `bench/sqlite-lifecycle-batching.ts`             | Durable SQLite `pushBatch` → `pullBatch` → `ackBatch` | Native before/after diagnostic; fresh database per warm-up/run, public `QueueManager` path, exact final counts and completed metrics, per-phase timings, five-run median by default; never publication evidence                                                                                                                                                                             |
...
- [Job Group Scheduling Benchmark — 2026-08-31](../benchmarks/job-groups-2026-08-31.md)
- [Native PostgreSQL 15–18 Engineering Benchmark — 2026-08-26](../benchmarks/postgres-versions-2026-08-26.md)
- [PostgreSQL 18 Multi-Broker Performance Analysis — 2026-08-26](../benchmarks/postgres-performance-analysis-2026-08-26.md)
- [Native Engineering Benchmark — 2026-08-03](../benchmarks/native-engineering-2026-08-03.md)
