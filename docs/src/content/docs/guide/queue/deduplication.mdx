...
```php
// Deduplication ids are stored as custom ids on the broker.
$job = $queue->getJobByCustomId('my-unique-key');
$jobId = $job?->id();
```
...
```go
// Deduplication ids are stored as custom ids on the broker.
job, err := queue.GetJobByCustomID("my-unique-key")
if err != nil {
    return err
}
```
...
```rust
// Deduplication ids are stored as custom ids on the broker.
let job = queue.get_job_by_custom_id("my-unique-key")?;
...
```elixir
# Deduplication ids are stored as custom ids on the broker.
{:ok, job} = Bunqueue.Queue.get_job_by_custom_id(queue, "my-unique-key")
...
to a replacement job. The network SDKs shown above can look up the owner through
the equivalent custom-id query; explicit key removal is currently Bun-only.*
