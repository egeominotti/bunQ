...
| Go 1.26.5 | `github.com/egeominotti/bunqueue/sdk/go` | 0.1.0 | `go get`, [source](https://github.com/egeominotti/bunqueue/tree/main/sdk/go) |
| Rust ≥ 1.85 | `bunqueue-client` | 0.1.0 | crates.io upcoming, [source](https://github.com/egeominotti/bunqueue/tree/main/sdk/rust) |
| Elixir ≥ 1.15 | `bunqueue_client` | 0.1.0 | Hex upcoming, [source](https://github.com/egeominotti/bunqueue/tree/main/sdk/elixir) |
| Bun, embedded in process, no server | `bunqueue` | - | [Quick Start](/guide/quickstart/) |

WebAssembly is a feasible target, but not one portable runtime. Browser WASM
has no standard raw-TCP API, so a browser SDK needs a bunqueue HTTP/WebSocket
bridge or a host-specific socket capability. WASI can reuse the Rust protocol
core only where the host grants sockets and TLS. A WASM target will become
official only when it can run the same conformance suite without weakening
authentication, TLS, framing, or telemetry guarantees.
...
All SDKs implement the core producer, lookup, wait, queue-control, DLQ,
...
Conformance is complemented by realistic end-to-end workloads for every
official SDK. These tests bulk-ingest distinct business records, process them
through the language's actual worker concurrency model, read every persisted
result by job id, and verify zero-loss checksums. Retry/DLQ, heartbeat,
graceful-shutdown, authentication, TLS, telemetry, and flow rollback scenarios
also run against disposable real brokers in `bun run test:sandbox:sdk`.

## Production test contract

Every official SDK passes the same layered contract:

| Layer | What is exercised |
|---|---|
| Functional | Unit, real-broker integration, complete E2E flows, 17 shared contract checks, and permanent bug regressions |
| Concurrency | Independent connections race one custom id and one available lease; worker slots, ACK/FAIL, and heartbeats stay bounded |
| Fault tolerance | Half-open timeouts, malformed frames, broker SIGKILL, durable restart visibility, lazy reconnect, TLS and authentication failures |
| Generated inputs | Fixed-seed portable payload invariants plus malformed/deep/cyclic/extension mutation corpora; Go adds native fuzzing |
| Capacity | Bounded 512-1500 job spikes in the normal gate and opt-in sustained load with configurable duration and batch size |
| Non-functional | Weekly compatibility, dependency-advisory, race, fuzz and 15-minute soak jobs |

The normal `bun run test:sandbox:sdk` run is isolated, offline and
deterministic. Hours-long soak and stress runs are opt-in:

```bash
# choose the matching tests/soak.* entrypoint for the SDK
BUNQUEUE_SDK_SOAK_SECONDS=3600 BUNQUEUE_SDK_SOAK_BATCH=100 bun sdk/typescript/tests/soak.ts
```

Increasing the batch explores backpressure and the practical breaking point;
it is not a portable benchmark. Publish performance numbers only from fresh
native processes, never Docker or a VM.

Database disk-full, WAL/power-loss and schema migration tests run in the
broker's persistence suite. A remote SDK cannot inject those filesystem
failures; its responsibility is typed failure, reconnect, durable visibility
and idempotent retry. Delivery is at-least-once, so a handler must still be
idempotent if it can crash after its side effects but before ACK.
