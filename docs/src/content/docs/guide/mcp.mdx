...
<Aside type="note">
  The JSON examples below use `args: ["--package=bunqueue", "bunqueue-mcp"]` so `bunx` resolves the bundled `bunqueue-mcp` binary straight from the `bunqueue` package — no separate install needed. If you prefer, run `bun add -g bunqueue` once and use a bare `args: ["bunqueue-mcp"]` instead.
</Aside>
...
4. Run `bunx bunqueue-mcp` manually to check for errors (a `404` for `bunqueue-mcp` means `bunqueue` isn't installed yet)
