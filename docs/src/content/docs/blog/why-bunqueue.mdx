...
<div class="bq-cards">
  <div class="bq-card">
    ...
    <p>No Redis, no external services. Just your app and a file on disk. <code>bun add bunqueue</code> installs 7 packages (5.5 MB), only <code>croner</code> and <code>msgpackr</code> at runtime.</p>
  </div>
  <div class="bq-card">
    <BqIcon name="speed" />
...
<Aside type="note" title="A genuinely small install">
  Eliminating Redis isn't just an operational win, it shows up in your `node_modules` too. As of 2.8.1, `bun add bunqueue` installs **7 packages (5.5 MB)** with just two runtime dependencies (`croner` and `msgpackr`). The MCP server's `@modelcontextprotocol/sdk` is now an optional peer dependency, loaded only if you run the `bunqueue-mcp` bin, so queue and worker users never download it.
</Aside>
