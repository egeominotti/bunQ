...
description: "bunqueue vs BullMQ compared honestly: throughput benchmarks with dated methodology, feature differences, and the cases where BullMQ is the better pick."
head:
  - tag: meta
    attrs:
      property: og:image
...
Pick **BullMQ** if you need to scale the queue itself across multiple servers, already operate Redis, or need workers in languages beyond bunqueue's official SDKs (TypeScript and Python). Details in [When to Use BullMQ Instead](#when-to-use-bullmq-instead).
...
<div class="bq-cards">
  ...
  <div class="bq-card">
    ...
    <p>Embedded SQLite. 7 packages, 5.5 MB installed.</p>
  </div>
</div>
...
<Aside type="note">
This is a fair comparison: both systems go over the network (bunqueue TCP vs Redis TCP), same payload size (100 bytes), same batch sizes, same push strategy. Embedded mode (queue inside your process, no network) benchmarks significantly higher; see [benchmarks](/guide/benchmarks/).
</Aside>
...
<Aside>
For single-process apps, a service plus its workers, or edge deployments, bunqueue gives you the same API with one less moving part.
</Aside>
