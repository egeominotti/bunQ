...
new Worker(
  ...
  async (job) => {
    ...
    stats.completed++;
    return {
      outputUrl: `https://cdn.acme.com/media/${Date.now()}.webp`,
      durationMs: rand(500, 8000),
    };
  },
...
// ~50 audit jobs/sec — user activity is the dominant volume.
  ...
  while (true) {
    const batch = Array.from({ length: rand(3, 6) }, () => ({
      ...
      data: {
        to: pick(users),
        userId: pick(userIds),
        template: pick(['welcome', 'invoice', 'reset', 'promo', 'alert']),
      },
      priority: rand(5, 20),
    }));
  ...
  while (true) {
    ...
    await Promise.all([
      ...
      auditQ.add('event', {
        userId,
        action: 'signup',
        ip: `34.${rand(0, 255)}.${rand(0, 255)}.${rand(1, 254)}`,
      }),
    ]);
    stats.added += 3;
...
// Media uploads — occasional
async function produceMediaUploads() {
  while (true) {
    await mediaQ.add(
      pick(['transcode', 'resize', 'thumbnail']),
      {
        userId: pick(userIds),
...
// Reports — infrequent and on demand
          ...
          [
            {
              name: 'charge',
              queueName: 'payments',
              data: { userId: pick(userIds), amount: pick(amounts), currency: pick(currencies) },
            },
            {
              name: 'inventory',
              queueName: 'data-import',
              data: { sku: `SKU_${rand(1000, 9999)}`, warehouse: pick(['EU', 'US', 'APAC']) },
            },
          ],
          {
            name: 'push',
            ...
            data: {
              ...
              body: `Your order #${rand(10000, 99999)} is confirmed`,
            },
          }
        );
        stats.added += 3;
        ...
        await flow.addChain([
          {
            name: 'verify',
            queueName: 'emails',
            data: { to: pick(users), template: 'verify-email' },
          },
          {
            name: 'setup',
            queueName: 'data-import',
            data: { userId: pick(userIds), step: 'profile-init' },
          },
          {
            name: 'guide',
            queueName: 'emails',
            data: { to: pick(users), template: 'getting-started' },
          },
        ]);
        stats.added += 3;
      } catch {}
  ...
  while (true) {
    if (maybe(activityMultiplier() * 0.3)) {
      try {
        ...
        await flow.addBulkThen(recipients, {
          name: 'payout',
          queueName: 'payments',
...
async function registerCrons() {
  try {
    await reportQ.upsertJobScheduler(
      'hourly-stats',
      { pattern: '0 * * * *' },
      { name: 'generate', data: { type: 'hourly-stats', auto: true } }
    );
  } catch {}
  try {
    await reportQ.upsertJobScheduler(
      'daily-report',
      { pattern: '0 7 * * *' },
      { name: 'generate', data: { type: 'daily-report', auto: true } }
    );
  } catch {}
  try {
    await reportQ.upsertJobScheduler(
      'weekly-digest',
      { pattern: '0 9 * * 1' },
      { name: 'generate', data: { type: 'weekly-digest', auto: true } }
    );
  } catch {}
  try {
    await emailQ.upsertJobScheduler(
      'promo-newsletter',
      { pattern: '0 10 * * 2,4' },
      { name: 'send', data: { template: 'newsletter', segment: 'active' } }
    );
  } catch {}
  try {
    await importQ.upsertJobScheduler(
      'nightly-sync',
      { pattern: '0 2 * * *' },
      { name: 'csv', data: { source: 'crm', type: 'full-sync' } }
    );
  } catch {}
  try {
    await auditQ.upsertJobScheduler(
      'audit-archive',
      { pattern: '0 1 * * *' },
      { name: 'archive', data: { olderThanDays: 90 } }
    );
  } catch {}
  try {
    await mediaQ.upsertJobScheduler(
      'media-cleanup',
      { pattern: '0 3 * * *' },
  ...
  try {
    await payQ.upsertJobScheduler(
      'payout-batch',
      { pattern: '0 16 * * 1-5' },
      { name: 'payout', data: { type: 'batch', currency: 'USD' } }
    );
  } catch {}
