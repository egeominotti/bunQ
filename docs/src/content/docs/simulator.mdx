---
title: "Interactive bunqueue Simulator: Try the Bun Job Queue in Your Browser"
description: "Try bunqueue live in your browser. Watch jobs flow through priorities, retries with backoff, dead letter queues, sharding, rate limits, and worker concurrency."
...
A faithful, in-browser model of the bunqueue engine. Push jobs and watch them move through the same lifecycle a real server runs: **waiting → active → completed**, with failures bouncing through **delayed** (exponential backoff) until they land in the **dead-letter lane**. Same FNV-1a shard mapping, same priority ordering, same controls.

Use the **scenario chips** for one-click demos, or drive everything by hand from the control panel. The transport bar pauses the simulation clock and runs it up to 4× speed.
