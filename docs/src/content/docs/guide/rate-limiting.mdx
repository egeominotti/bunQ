...
```typescript
await queue.setGlobalRateLimit(100);    // 100 jobs per second
await queue.removeGlobalRateLimit();
```

*The TypeScript TCP SDK does not send a custom `duration` yet, so the window is always 1 second; set a custom window from the Bun client or the PHP, Go, Rust, or Elixir SDK.*
...
*The Python SDK does not send a custom duration yet, so the window is always 1 second; set a custom window from the Bun client or the PHP, Go, Rust, or Elixir SDK.*
...
*The worker `limiter` option is available in the Bun client. In the TCP SDKs, use the queue-level rate limit above (which supports any window) or a client-side limiter of your own in the processor.*
