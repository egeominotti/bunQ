...
describe('complete CLI functionality over the real executable', () => {
  test('core and every job subcommand', async () => {
    const q = queue('job');
  ...
  test('every queue and limiter subcommand', async () => {
    const q = queue('queue');
  ...
  test('every cron, worker and webhook subcommand', async () => {
    const q = queue('admin');
    await cliOk([
      'cron',
      'add',
  ...
  test('every monitoring command', async () => {
    await cliOk(['stats']);
    await cliOk(['metrics']);
    await cliOk(['health']);
    await cliOk(['ping']);
