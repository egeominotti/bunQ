...
await queue.setGlobalRateLimit(100, 60_000);  // 100 jobs per minute
...
The TypeScript network SDK has no per-worker limiter. Use a custom queue-wide
broker window instead:

```typescript
await queue.setGlobalRateLimit(100, 60_000); // 100 starts/min across all workers
```
