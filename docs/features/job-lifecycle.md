...
3. `tryDequeueNextJob` (`pull.ts:50`): `peek()` head; if expired (`isExpired`) pop, decrement, delete from index, emit `job:expired`, and **skip** to next; if not ready (`isReady` false → delayed) **stop**; if `groupId` is active **stop** (FIFO group blocking). Otherwise `pop()`, `activateGroup`, set `startedAt`/`lastHeartbeat = now`, append timeline `active`, **insert into `processingShards[procIdx]` and set `jobIndex` to `{ type: 'processing', shardIdx: procIdx }` in the same synchronous critical section as the pop**, then return the job. The atomic queue -> processing transition is what keeps `getJob`/`getJobState` from ever observing a `'queue'` index entry for a job that is no longer findable in the shard queue (false-null flicker; see job-queries-and-control.md). The `processingShards` insert is deliberately lock-free: the key is brand new to the map, a synchronous `Map.set` cannot interleave with a paused critical section, and taking the async `processingLocks` RWLock here would hold the hot shard write lock across an await.
