...
These external SDKs currently compose `PUSH`/`UpdateParent` calls. If creation
fails they attempt to cancel jobs already created, but that cleanup is
best-effort and is not a transaction: a worker can observe an early leaf, and a
cleanup failure can leave a partial graph. The Bun package (`bunqueue/client`)
uses one broker-side `PUSHF` command and has the stronger all-or-nothing
guarantee.

External-SDK `getFlow(id)` returns `null` for a missing root and skips children
removed since creation, yielding the surviving partial tree. The Bun package
