...
<div class="bq-wrap bq-hero">
  ...
  <h1 class="bq-hero-h1 bq-bench-h1">
    BullMQ code, minus <em>Redis.</em>
  </h1>
  <p class="bq-hero-sub">
    bunqueue keeps the BullMQ Queue and Worker API. Most migrations are an import change and a
    deleted Redis config. This page shows the diff first, then lists every real difference.
  </p>
</div>
...
_Embedded mode (`embedded: true`) ships in the Bun `bunqueue` package only. On Node.js and Deno, `bunqueue-client` connects to a bunqueue server; the Queue and Worker API is otherwise the same._
...
- **Flows**: `FlowProducer.add()`, `addBulk()`, `getFlow()`,
  ...
  package sends one broker-side `PUSHF` graph; configured SQLite commits all
  rows before publication, providing the same all-or-nothing creation
  guarantee without client-side rollback.
...
bunqueue uses one SQLite broker by default, and many workers may connect over
TCP. If the broker tier must scale, standalone servers can instead share a
PostgreSQL 18.6 database/namespace. This is not Redis Cluster and does not copy
...
- [ ] For multi-broker server mode, PostgreSQL 18.6 URL/namespace and unique broker IDs configured
