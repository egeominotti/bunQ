...
`SandboxedWorker` is marked **experimental by bunqueue** and most of its historical
test suites are currently disabled. Treat it as an opt-in Bun-only feature, pin
