...
<Aside type="caution" title="The MCP server needs @modelcontextprotocol/sdk (v2.8.0+)">
  Since v2.8.0, `@modelcontextprotocol/sdk` is an **optional peer dependency** of `bunqueue`, not a regular dependency. It's only used by the MCP server, so every queue-only user no longer downloads it. A plain `bun add bunqueue` now installs **7 packages instead of 117** and ships **8.2 MB of `node_modules` instead of 93 MB** — a **91% smaller install** that's also 3.8× faster to install cold (0.72s vs 2.73s). The trade-off: when you *do* run the MCP server, you install the SDK yourself, once:
