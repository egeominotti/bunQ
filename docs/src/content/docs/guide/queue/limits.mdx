...
await queue.setGlobalRateLimit(100, 60_000);  // max 100 jobs per minute
...
- `getRateLimitTtl(maxJobs?)` returns `-2` when no rate limit exists; otherwise it reports the temporary-limit lifetime or token wait.
