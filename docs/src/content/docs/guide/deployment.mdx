...
<div class="bq-wrap bq-hero">
  ...
  <p class="bq-hero-sub">How to run bunqueue in production as the single-instance job queue it is designed to be. Docker, systemd and PM2 setups, S3 backups, health checks, resource limits and disaster recovery.</p>

  <div class="bq-proof">
    <span><b>1M</b> jobs validated, 100% integrity</span>
    <span><b>3</b> deployment options</span>
    <span><b>S3</b> backups for disaster recovery</span>
    <span><b>1</b> process, no clustering</span>
  </div>
</div>
...
<div class="bq-diag">
  <div class="bq-diag-group">
    <span class="bq-diag-group-label">Your Application</span>
    <div class="bq-diag-row">
      <div class="bq-diag-cell">Web App</div>
      <div class="bq-diag-cell">API</div>
      <div class="bq-diag-cell">Workers</div>
    </div>
    <div class="bq-diag-arrow">↓</div>
    <div class="bq-diag-layer">bunqueue <i>embedded mode, single instance</i></div>
    <div class="bq-diag-arrow">↓</div>
    <div class="bq-diag-layer bq-diag-accent">SQLite Database <i>./data/bunq.db, local file</i></div>
  </div>
  <div class="bq-diag-arrow">↓</div>
  <div class="bq-diag-layer">S3 Backup <i>optional, disaster recovery</i></div>
</div>
...
### Option 2: Separate Worker Process (through the server)

Run your API and workers as separate processes. Multi-process setups always go through the bunqueue server: it owns the SQLite file, and every process talks to it over TCP.

```bash
# One server owns the database
bunqueue start --data-path ./data/bunq.db
```

```typescript
// api.ts - Your web server (TCP client, no embedded flag)
...
```typescript
// worker.ts - Separate process (TCP client)
...
**Pros:**
- Workers can be restarted and scaled independently
- Better resource isolation

**Cons:**
- One more process to manage (the server)
- All traffic goes over TCP (localhost round trip)

<Aside type="danger" title="Never share one SQLite file between two embedded processes">
Two processes with `embedded: true` pointing at the same `DATA_PATH` do **not** work. Each process keeps its own in-memory queue state and reads SQLite only once at startup, so a worker process never sees jobs pushed live by the API process, and both run independent cleanup and buffered writes against the same file. If you need more than one process, use the server as shown above.
</Aside>
...
# Health check (wget ships with the Alpine base, curl does not)
...
To update bunqueue on a systemd-managed server:

```bash
# 1. Stop the service
sudo systemctl stop bunqueue
...
curl http://localhost:6790/health | jq .version
```

:::caution
Always verify the running version with `curl /health` after restart. Simply running `bun update` does **not** restart the service, you must explicitly stop and start it with `systemctl`.
:::
...
# {"ok":true,"status":"healthy","uptime":3600,"version":"2.8.30",
...
For failure semantics, the crash timeline and the invariants each adversarial test suite proves, see [Production Operations](/guide/production/).
...
- ✅ Multiple worker processes on the same machine (through the TCP server, never by sharing the SQLite file)
...
<div class="bq-diag">
  <div class="bq-diag-flow">
    <div class="bq-diag-cell">Every 1 hour</div>
    <div class="bq-diag-arrow">→</div>
    <div class="bq-diag-cell bq-diag-accent">S3 backup</div>
  </div>
  <div class="bq-diag-flow">
    <div class="bq-diag-cell">Every 6 hours</div>
    <div class="bq-diag-arrow">→</div>
    <div class="bq-diag-cell">Verify backup integrity</div>
  </div>
  <div class="bq-diag-flow">
    <div class="bq-diag-cell">Every day</div>
    <div class="bq-diag-arrow">→</div>
    <div class="bq-diag-cell">Test restore in staging</div>
  </div>
</div>
...
SQLite WAL mode allows recovery to recent states. Stop the server first: copying the database and WAL while bunqueue is writing can produce a torn, unusable copy.

```bash
# Stop, then copy both files together
systemctl stop bunqueue
...
- Terminate TLS natively with `TLS_CERT_FILE` and `TLS_KEY_FILE` (both TCP and HTTP, see [Native TLS](/guide/tls/)), or run behind a reverse proxy (nginx, Caddy)
