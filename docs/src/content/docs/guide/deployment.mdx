---
title: 'Deploy bunqueue: SQLite or PostgreSQL 18.6'
description: Deploy bunqueue with embedded or single-broker SQLite, or a PostgreSQL 18.6 multi-broker topology, including Docker, health, and backups.
head:
  - tag: meta
    attrs:
      property: og:image
...
<div class="bq-wrap bq-hero">
  ...
  <p class="bq-hero-sub">
    ...
    configs for Docker, systemd, and PM2, plus health checks and backups.
  </p>
</div>

bunqueue uses one process and memory/SQLite by default. Server mode may instead
use PostgreSQL 18.6 as the authoritative store for multiple active broker
processes. Start by deciding whether the queue is embedded, a single SQLite
server, or a PostgreSQL-backed broker fleet.
...
## Multiple active brokers: PostgreSQL 18.6
...
different IDs. Configure the equivalent deployment with:
...
- **SQLite is single-broker.** Horizontal broker scaling requires PostgreSQL
  18.6 mode. PostgreSQL/database HA, routing, and backup remain operational
