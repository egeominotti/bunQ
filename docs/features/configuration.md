...
> **Category:** Infrastructure · **Source:** `src/config/resolve.ts`, `src/config/types.ts`, `src/config/loader.ts`, `src/config/index.ts`, `src/main.ts`, `src/require-bun.ts`, `src/bun-only.ts`, `src/shared/logger.ts`, `src/shared/version.ts`

## Purpose
This module is the configuration layer and process entrypoint of the server. It auto-discovers an optional `bunqueue.config.{ts,js,mjs}` file, merges it with environment variables and built-in defaults (config file wins), and produces strongly-typed resolved config objects (`ResolvedConfig`, `CloudConfig`, `S3BackupConfig`, TLS options) consumed by the server bootstrap. It also owns the `bunqueue` executable's top-level dispatch (bare invocation → server, anything else → CLI), the structured `Logger`, the package `VERSION`, and the Bun-only runtime guards that fail fast under Node.
...
interface ResolvedConfig {
  tcpPort: number;                 httpPort: number;
  hostname: string;
  tcpSocketPath: string | undefined;   httpSocketPath: string | undefined;
  tlsCertFile: string | undefined;     tlsKeyFile: string | undefined;
  authTokens: string[];
...
## Concurrency & Locking
N/A. This module is synchronous config resolution plus one-shot dispatch; it holds no locks. The graceful-shutdown re-entrancy guard (`shuttingDown` flag) and the active-job drain loop live in `bootServer` (`bootstrap.ts:156-192`), not here.
