...
<div class="bq-wrap bq-hero">
  ...
  <h1 class="bq-hero-h1 bq-bench-h1">
    Working queue in <em>a minute.</em>
  </h1>
  <p class="bq-hero-sub">
    Create a queue, add jobs, process them with a Worker, and turn on persistence. On Bun everything
    runs embedded in a single process with zero configuration; from any other language, start the
    server once and connect.
  </p>
</div>
...
_Rust and Elixir have no worker event emitter: use the structured telemetry callback for transport lifecycle and normal language control flow for per-job outcomes (see the [SDK guide](/guide/sdks/))._
...
_`dataPath` applies to embedded (Bun) mode only. In server mode, persistence is configured on the server (`bunx bunqueue start --data-path ./data/bunq.db`); clients in every language need no changes._
...
```typescript
process.on('SIGINT', async () => {
  await worker.close(); // Stop pulling, drain in-flight jobs
...
_Simple Mode is available in TypeScript and Python. In PHP, Go, Rust and Elixir, compose `Queue` and `Worker` directly._
