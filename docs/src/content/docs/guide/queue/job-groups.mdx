...
PostgreSQL `addBulk` is one transaction. The embedded (memory/SQLite) `addBulk`
instead follows the accepted-prefix contract: jobs before the one that hits the
