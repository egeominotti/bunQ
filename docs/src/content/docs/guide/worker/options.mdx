...
| `skipStalledCheck` | `boolean` | `false` | In embedded mode, skip only this Worker's subscription to `stalled` notifications. It does not disable broker-side stall detection or recovery |
