...
<div class="bq-cards">
  ...
  <div class="bq-card">
    <BqIcon name="dials" />
    <h3>2 Modes</h3>
    <p>Embedded (local SQLite, zero config) or TCP (connect to a remote bunqueue server)</p>
  </div>
</div>
...
2. **Install the MCP SDK**, since v2.8.0, `@modelcontextprotocol/sdk` is an **optional peer dependency**: it's only needed to run the MCP server, so queue-only users don't download it. Install it once in the same project/environment where the server runs:
...
<Aside type="caution" title="The MCP server needs @modelcontextprotocol/sdk (v2.8.0+)">
  Since v2.8.0, `@modelcontextprotocol/sdk` is an **optional peer dependency** of `bunqueue`, not a regular dependency. It's only used by the MCP server, so every queue-only user no longer downloads it. A plain `bun add bunqueue` now installs **7 packages instead of 117** and ships **5.5 MB of `node_modules` instead of 93 MB**, a **94% smaller install** that's also up to ~5× faster on a cold install. The trade-off: when you *do* run the MCP server, you install the SDK yourself, once:
...
4. Install the optional MCP SDK peer dependency: `bun add @modelcontextprotocol/sdk` (since v2.8.0 it's not bundled; a missing-SDK launch exits with `[bunqueue-mcp] The MCP server requires "@modelcontextprotocol/sdk"`)
