...
<div class="bq-wrap bq-hero">
  ...
  <p class="bq-hero-sub">Official client SDKs for Node.js, Deno, Bun, Python, PHP, Go and Cloudflare Workers. Each one speaks the native TCP protocol, msgpack and pipelined, certified against a formal wire spec by a language-agnostic conformance suite.</p>

  <div class="bq-proof">
    <span><b>110</b> e2e scenarios each on Node, Deno, Bun</span>
    <span><b>100</b> in Python · <b>33</b> in PHP · <b>32</b> in Go</span>
    <span><b>16</b> scenarios inside workerd</span>
    <span><b>17</b> conformance checks on 1 versioned wire protocol</span>
  </div>
</div>
...
The protocol itself is a formal, public contract: the
[wire protocol specification](https://github.com/egeominotti/bunqueue/blob/main/docs/protocol.md)
plus a language-agnostic
[conformance suite](https://github.com/egeominotti/bunqueue/tree/main/sdk/conformance)
of 17 checks that every official SDK passes against a real server. A
community client in any language becomes protocol-conformant the same way:
write a small driver, run the suite, pass 17/17.

Every claim on this page is verified against a real bunqueue server, on
every supported platform: **110 end-to-end scenarios each on Node.js, Deno
and Bun**, 16 inside workerd (the actual Cloudflare Workers runtime),
**100 in Python**, **33 in PHP** and **32 in Go**, plus the 17-check
conformance suite per SDK, with npm releases published with build
provenance and gated on the full e2e suite.
...
The TCP connect timeout is internal in both SDKs (5 seconds) and is not
configurable through the constructor; the command timeout above governs
how long each in-flight command waits for a response.
...
| `heartbeatIntervalS` | 10 | Worker + per-job lock heartbeats; 0 disables |
...
semantics (Python: same methods in snake_case; the rare name that differs
is noted inline).
...
|---|---|
| Node.js | 110 e2e scenarios and 8 integration tests |
| Bun | 110 e2e scenarios and 8 integration tests |
| Deno | 110 e2e scenarios and 8 integration tests, identical test files |
| Cloudflare Workers | 16 scenarios executed inside workerd, full API surface and Simple Mode |
| Python | 100 e2e scenarios and 8 integration tests |
| PHP | 33 e2e scenarios against a real server, plus a dedicated auth server |
| Go | 32 e2e scenarios, `go vet` and `gofmt` clean |
| Every SDK | 17-check [conformance suite](https://github.com/egeominotti/bunqueue/tree/main/sdk/conformance), double-verified through an independent wire connection |
...
- [Conformance suite](https://github.com/egeominotti/bunqueue/tree/main/sdk/conformance), certify a client in any language with 17 checks
