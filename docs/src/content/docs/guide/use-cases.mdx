...
<div class="bq-wrap bq-hero">
  ...
  <h1 class="bq-hero-h1 bq-bench-h1">What teams run <em>on it.</em></h1>
  <p class="bq-hero-sub">API requests should return immediately while emails, images, payments and external calls happen in the background. Every pattern below is production ready, runs in embedded mode with zero external services, and gets retries, DLQ, progress tracking and cron scheduling from the queue itself.</p>
</div>
...
```bash
# bunqueue-mcp is a binary bundled with bunqueue, install it first, then connect Claude Code
bun add bunqueue
...
The MCP server runs as a subprocess, communicates via stdio using JSON-RPC, and supports both embedded mode (in-process, SQLite persistence via `DATA_PATH`) and TCP mode (connect to a remote bunqueue server). See the [MCP Server guide](/guide/mcp/) for full setup, all 73 tools, HTTP handler configuration, and real-world examples.
...
const notificationWorker = new Worker<NotificationJob>('notifications', async (job) => {
  ...
  for (const channel of allowedChannels) {
    try {
      ...
      await job.log(`Failed ${channel}: ${(err as Error).message}`);
    }
  }
...
const paymentQueue = new Queue<PaymentJob>('payments', {
  embedded: true,
  dataPath: './data/payments.db', // durable writes need SQLite persistence
  defaultJobOptions: {
    attempts: 3,
    backoff: 5000,
    timeout: 60000,
...
await paymentQueue.add('charge', {
  ...
  idempotencyKey: 'order-ORD-123-payment',
...
<Aside type="note" title="Works in embedded mode too">
Cron scheduling via `upsertJobScheduler()` works in **both embedded and server mode**. Schedules are persisted in SQLite and survive restarts. You can also attach a `repeat` option to `queue.add()`. See the [Cron guide](/guide/cron/).
</Aside>

Recurring tasks using cron expressions or plain intervals, in either mode.
...
// Daily cleanup at 3 AM, cron expression persisted in SQLite
await scheduledQueue.upsertJobScheduler('daily-cleanup',
  { pattern: '0 3 * * *' },
...
<Aside type="note" title="Works in embedded mode too">
Rate limiting works in **both embedded and server mode**. Use `queue.setGlobalRateLimit(max, duration?)`, a per-worker `limiter: { max, duration }` in `WorkerOptions`, or control throughput with worker `concurrency` and job `backoff`.
</Aside>

<div class="rl-modes">
  <div class="rl-mode">
    <span class="rl-tag rl-tag-server">Queue-level</span>
    <code>setGlobalRateLimit(100)</code>
    <span class="rl-arrow">→</span>
    <span class="rl-desc">Token bucket (~100/sec)</span>
  </div>
  <div class="rl-mode">
...
const mergeWorker = new Worker('processing', async (job) => {
  if (job.name === 'merge-results') {
    ...
    // { 'processing:<childId>': { chunk: 1, ... }, ... }  (Record<jobKey, returnValue>)
