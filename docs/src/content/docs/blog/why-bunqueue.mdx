...
<CardGrid>
  <Card title="Zero Dependencies" icon="rocket">
    No Redis, no external services. Just your app and a file on disk. `bun add bunqueue` installs 7 packages (8.2 MB) — only `croner` and `msgpackr` at runtime.
  </Card>
...
<Aside type="note" title="A genuinely small install">
  Eliminating Redis isn't just an operational win — it shows up in your `node_modules` too. As of 2.8.0, `bun add bunqueue` installs **7 packages (8.2 MB)** with just two runtime dependencies (`croner` and `msgpackr`). The MCP server's `@modelcontextprotocol/sdk` is now an optional peer dependency, loaded only if you run the `bunqueue-mcp` bin — so queue and worker users never download it.
</Aside>
