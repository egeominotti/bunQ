...
filter. `RemoveDlqJob { queue, jobId }` permanently deletes one failed job and
returns `data.removed`; a missing entry is `ok: true` with `removed: false`,
while persistence/handler failures use the normal error response.
`RetryCompleted` accepts `id?`, `count?`, and `timestamp?`.
...
`PULL`/`PULLB` carry optional positive-safe-integer group defaults. The fourteen
group getter/control commands expose server-authoritative depth, active count,
fixed-window TTL/configuration, concurrency, pause state, and immediate manual
deadlines. Grouped `PUSH`/`PUSHB`/`PUSHF` inputs carry normalized `groupId`,
ascending BullMQ Pro priority, and optional atomic `groupMaxSize`; a failed
capacity check rejects the complete batch/graph through the ordinary error
response.
...
- **Frame size cap (64MB):** a declared length over `MAX_FRAME_SIZE` throws `FrameSizeError`, clears the parser buffer (stops processing attacker bytes), sends an error, and ends the socket. Legal large partial frames are _not_ dropped mid-assembly — only over-cap declarations are.
...
- **MessagePack map-key preservation:** JSON-like maps use string keys.
  ...
  non-JavaScript SDKs.
- **Error sanitization:** handler and transport boundaries redact SQLSTATE,
