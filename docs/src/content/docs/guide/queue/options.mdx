...
`group.maxSize` must be a positive safe integer and rejects the entire single,
bulk, or flow admission before writes when the group's pending depth is full.
