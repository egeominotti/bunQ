...
Pass the secret directly through Bun's `NPM_CONFIG_TOKEN` environment variable.
The pinned Bun authentication check does not pick up the indirect
`NPM_CONFIG_USERCONFIG`/`NODE_AUTH_TOKEN` setup generated by `setup-node`.
