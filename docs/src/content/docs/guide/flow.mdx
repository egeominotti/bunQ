...
In the Bun package, flow creation is one broker-side transaction: `addBulk`
commits every tree or none, and workers cannot see a leaf before the full graph
exists. The external language SDKs shown above remain compatible with the same
server, but releases that still compose `PUSH` and `UpdateParent` use
best-effort cleanup rather than the Bun package's `PUSHF` transaction.
...
- `jobId` is allowed, but cannot be empty or contain `:`. Reusing any existing
  ...
  whole request.
- User data keys named `name` or beginning with `__` are reserved for flow
  metadata.
...
runs first, then its children), within the creation limits above:
