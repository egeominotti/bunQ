...
describe('Cron Timezone & DST Edge Cases', () => {
  ...
  test('same cron yields different UTC times for EST vs EDT periods', () => {
    ...
    expect(new Date(getNextCronRun('0 9 * * *', winter, 'America/New_York')).getUTCHours()).toBe(
      14
    );
    expect(new Date(getNextCronRun('0 9 * * *', summer, 'America/New_York')).getUTCHours()).toBe(
      13
    );
  });
  ...
  test('six-field seconds preserve native DST resolution', () => {
    const beforeRomeGap = Date.parse('2024-03-30T12:00:00Z');
    const beforeNewYorkOverlap = Date.parse('2024-11-02T12:00:00Z');

    expect(getNextCronRun('30 30 2 * * *', beforeRomeGap, 'Europe/Rome')).toBe(
      Date.parse('2024-03-31T01:30:30Z')
    );
    expect(getNextCronRun('30 30 1 * * *', beforeNewYorkOverlap, 'America/New_York')).toBe(
      Date.parse('2024-11-03T05:30:30Z')
    );
  });
});
