---
title: "Cron Scheduling: Recurring Jobs for Bun Apps"
...
head:
  - tag: meta
    attrs:
      property: og:image
      content: https://bunqueue.dev/og/blog/cron-scheduling.png
  - tag: meta
    attrs:
      property: og:type
      content: article
...
<div class="bq-wrap bq-hero">
  <span class="bq-eyebrow">blog · scheduling</span>
  <h1 class="bq-hero-h1 bq-bench-h1">Cron, without the <em>crontab.</em></h1>
  <p class="bq-hero-sub">Daily reports, hourly cleanups, periodic syncs. bunqueue ships a built-in cron scheduler with expressions, repeat intervals, timezones, and execution limits, running alongside your queue with zero extra infrastructure.</p>
</div>
...
<Aside type="note">
  `every` is millisecond-based and doesn't drift. The scheduler is fixed-rate: the next run is anchored to the previous scheduled slot, so a slow or late execution does not push the schedule forward cumulatively.
</Aside>
...
# Add a cron job (name is positional)
bunqueue cron add daily-report --queue reports \
