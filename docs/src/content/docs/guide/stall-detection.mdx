...
than 30 seconds, or when you need a different recovery budget.
...
The Bun `Worker` also emits `stalled` in embedded mode. TCP workers do not;
stall events originate in the broker after the original worker may already be
gone. The SSE or WebSocket stream is the portable live channel. Stall webhooks
are not emitted, so do not register `job.stalled` as a webhook event.
...
const stalledJobs = entries.filter((entry) => entry.reason === 'stalled');
```
...
stalled_jobs = [entry for entry in entries if entry.get("reason") == "stalled"]
```
...
$stalledJobs = array_filter(
    $entries,
    fn ($entry) => ($entry['reason'] ?? null) === 'stalled'
);
```
...
```go
entries, err := queue.GetDlq(0) // 0 means no explicit count bound
stalledJobs := make([]map[string]any, 0)
for _, entry := range entries {
    if entry["reason"] == "stalled" { stalledJobs = append(stalledJobs, entry) }
}
```
...
```rust
// Entries are MessagePack maps; inspect each map's `reason` field.
...
stalled_jobs = Enum.filter(entries, &(&1["reason"] == "stalled"))
```
