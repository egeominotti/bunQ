...
The synchronous query API on this page (`getDlq()`, `getDlqStats()`,
`retryDlqByFilter()`) reads in-process state and therefore remains embedded-only;
the synchronous TCP mutation forms are fire-and-forget. The `Async` variants
work in both modes: `getDlqAsync(filter?)` returns full metadata and operational
Job objects, `getDlqStatsAsync()` returns authoritative statistics,
`retryDlqByFilterAsync(filter)` applies server-side filtering, and the existing
retry/purge/config async forms wait for the real result. `getDlqJobsAsync(count?)`
remains the compact jobs-only view.
:::
