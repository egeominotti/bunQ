...
@test
def broker_timeout_suppresses_late_worker_events(server: Server) -> None:
    with Queue(unique_name("q"), port=server.port) as queue:
        completed = []
        failed = []
        errors = []
        ...
        def process(job):
            time.sleep(0.5)
            if job.data["fail"]:
                raise RuntimeError("late processor failure")
        ...
        worker.on("completed", lambda job, _result: completed.append(job.id))
        worker.on("failed", lambda job, _error: failed.append(job.id))
        ...
        try:
            ...
            assert wait_until(
                lambda: queue.get_state(success.id) == "failed"
                and queue.get_state(failure.id) == "failed",
                15,
            )
            time.sleep(0.7)
            assert completed == [], "late result must not emit completed"
            assert failed == [], "late throw must not emit failed"
            assert errors == [], "authoritative ignored outcomes are not Worker errors"
        finally:
            worker.close(timeout=10)
