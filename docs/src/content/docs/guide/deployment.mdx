...
- **SQLite jobs you cannot afford to lose in a hard crash should be `durable: true`.** SQLite buffers ordinary writes for up to 10 ms; `durable` writes hit disk before the add returns. PostgreSQL mutations are already transactional and do not use this buffer. Details in [Production Operations](/guide/production/).
