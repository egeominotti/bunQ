...
description: "The experimental bunqueue SandboxedWorker runs handlers in worker threads on Bun, Node.js, and Deno so CPU-heavy jobs cannot block the main loop."
head:
  - tag: meta
    attrs:
      property: og:image
