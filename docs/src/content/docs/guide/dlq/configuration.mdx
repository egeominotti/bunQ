...
*The DLQ policy is server-side state per queue: set it once from any client and it governs jobs failed by workers in every language. The typed helper ships in the Bun package, the TypeScript SDK and the Python SDK; from PHP, Go, Rust and Elixir use `PUT /queues/:queue/dlq-config` over HTTP, or the `SetDlqConfig` TCP command.*
