...
### PULL (`pullJob`, `pull.ts:205`)

1. `deadline = timeoutMs > 0 ? now + timeoutMs : 0`. Loop:
2. `tryPullFromShard` (`pull.ts:245`) under `shardLocks[idx]`: return `null` if `state.paused`; then `tryAcquireRateLimit` and `tryAcquireConcurrency` gate (each rejection emits a dashboard event and returns null). Then loop `tryDequeueNextJob`.
3. `tryDequeueNextJob` (`pull.ts:50`): `peek()` head; if expired (`isExpired`) pop, decrement, delete from index, emit `job:expired`, and **skip** to next; if not ready (`isReady` false → delayed) **stop**; if `groupId` is active **stop** (FIFO group blocking). Otherwise `pop()`, `activateGroup`, set `startedAt`/`lastHeartbeat = now`, append timeline `active`, return the job.
4. `moveToProcessing` (`pull.ts:103`): under `processingLocks[procIdx]` insert into `processingShards`, set `jobIndex` to `{ type: 'processing', shardIdx: procIdx }`, `storage.markActive(...)` (non-fatal on error — in-memory is source of truth), broadcast `pulled`.
5. If no job and deadline not reached, `await shard.waitForJob(remaining)` and loop; otherwise return `null`.

`pullJobBatch` (`pull.ts:282`) pulls up to `count` jobs in one shard lock, acquiring a rate-limit + concurrency slot per job and releasing the slot when a dequeue yields `stop`/`skip` (`pull.ts:357`).

**Safety net:** if `moveToProcessing` throws, `requeueJob` (`pull.ts:179`) removes the job from `processingShards`, resets `startedAt = null`, releases the group + concurrency slot, pushes it back to the queue, and returns `null` so the job is never lost.
...
Lock-token verification, the **#101 grace window** (`isExpiredButOwned`: an expired-but-still-ours lock on a still-processing job is accepted, not lost), and stall-retry recovery live in `QueueManager.ack`/`fail` (`queueManager.ts:350`, `:496`), wrapping these raw functions. `WaiterManager` (`waiterManager.ts`) is single-threaded-event-loop safe: `notify()` wakes the first non-cancelled waiter or increments `pendingNotifications`; `waitForJob` returns immediately while a pending notification exists; cancelled waiters are reclaimed lazily and fully compacted past `WAITERS_CLEANUP_THRESHOLD = 1000`.
